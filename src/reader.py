    ...
    def read_lgp_data(rld_rec,rld_base):
        ...
        try:
            ...
            for li in range(10,18):
                data[li]                                    =   parsed[0][li]
            # R12 «данные» отдаём сырым внутренним значением 1С вместе с обрамляющими {},
            # иначе ЗначениеИзСтрокиВнутр в обработке epf/Nikita не десериализует его.
            # Регэксп sel_re (группа 13) захватывает содержимое без внешних скобок — возвращаем их.
            data[12]                                        =   '{' + parsed[0][12] + '}'
