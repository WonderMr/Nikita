...
Приложение считывает настройки из `Nikita.ini` и переменных окружения (поддерживается файл `.env`).
...
2.  Отредактируйте `.env`, указав параметры подключения:
    ```ini
...
3.  При первом запуске будет сгенерирован файл `Nikita.ini` с настройками по умолчанию. Там можно тонко настроить пути к кластеру 1С и количество потоков.
