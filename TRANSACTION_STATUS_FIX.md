...
**ВАЖНО: Функция УСТАРЕЛА и больше НЕ применяет инверсию!**

**Историческая ошибка:**
Ранее считалось, что в старом формате LGP статусы транзакций записаны НАОБОРОТ, и применялась инверсия:
- C → U (Зафиксирована → Не завершена) ❌
- R → C (Отменена → Зафиксирована) ❌
- U → R (Не завершена → Отменена) ❌

**Реальность:**
Статусы в LGP записаны **ПРАВИЛЬНО** согласно документации 1С:
- `C` = Commit → Зафиксирована ✅
- `R` = Rollback → Отменена ✅
- `U` = Unfinished → Не завершена ✅
- `N` = No transaction → Нет транзакции ✅
...
**Текущее состояние:**
- `fix_act_tran()` просто возвращает запись без изменений (сохранена для обратной совместимости)
- `reader.trans_descr()` вызывается БЕЗ параметра `fix=True`
- `tran_fix_list` больше не используется

**Почему была путаница:**
Изначально применялась инверсия только к событиям из `tran_fix_list`, что создавало впечатление, 
что это "исправление бага". На самом деле это было ошибкой - статусы всегда были правильными.
