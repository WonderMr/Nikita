...
Функция ПолучитьИмяСервера(СерверЖР)
    Если  СерверЖР                          =   "" Тогда
        ...
        Если Сервер                         =   Неопределено Тогда
            ...
            ПозицияМаркера = Найти(СтрокаПодключения,""";REF=");
            Если ПозицияМаркера > 0 Тогда
                Сервер                      =   Лев(СтрокаПодключения,ПозицияМаркера-1);
...
Процедура ОтключитьОтбор()
    ...
    ОтборЖурналаРегистрации = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(ОтборЖурналаРегистрацииПоУмолчанию));
    Критичность = "ВсеСобытия";
    ОбновитьТекущийСписок();
